...
(defn- catalog [entities] {:entities entities :collection-count (count entities)})

(defn- bare-entity [id nm kind]
  {:id id :name nm :kind kind :description nil
   :field-count 0 :fields [] :measure-names []})

(deftest ^:parallel fixture-1-scores-deterministically-test
  (testing "loading fixture_1 and scoring produces the v3 dimension breakdown we designed it for"
    ...
    (let [result (#'cli/run-cli {:representation-dir fixture-1-dir :level 2})]
      (testing "library score matches the hand-derived v3 total"
        (is (=? {:total 223
                 :dimensions
                 {:scale    {:variables {:entity-count         {:value 6 :score 60}
                  ...
                  :semantic {:variables {:synonym-pairs {:value 1 :score 50}}
                             :sub-total 50}}}
                (:library result))))
      (testing "universe score matches the hand-derived v3 total"
        (is (=? {:total 408
                 :dimensions
                 {:scale    {:variables {:entity-count         {:value 9 :score 90}
                                         :field-count          {:value 5 :score 5}
                                         :collection-tree-size {:value 4 :score 4}}
                             :sub-total 99}
                  :nominal  {:variables {:name-collisions        {:value 2 :score 200}
                                         :repeated-measures      {:value 2 :score 4}
                                         :field-level-collisions {:value 1 :score 5}}
                             :sub-total 209}
...
(deftest ^:parallel level-override-test
  (testing "--level 1 skips the :semantic dimension and doesn't invoke the embedder"
    (let [result (#'cli/run-cli {:representation-dir fixture-1-dir :level 1})]
...
(deftest ^:sequential run-cli-writes-readable-edn-to-output-file-test
  (testing "--output path gets a readable EDN dump of the same result"
    (let [tmp (doto (java.io.File/createTempFile "complexity-cli-output-" ".edn") .deleteOnExit)]
      (#'cli/write-result! (#'cli/run-cli {:representation-dir fixture-1-dir :level 2})
                           (.getAbsolutePath tmp))
      (is (= 223 (-> (slurp tmp) edn/read-string :library :total))))))
...
(deftest ^:parallel raw-embedding-keys-normalize-through-offline-scorer-test
  (testing "End-to-end: embeddings.json with mixed-case/whitespace-padded keys still produces the
            expected synonym match after load-dir + run-cli."
    ...
    (let [tmp-dir (doto (java.io.File/createTempFile "raw-keys-rep-" "")
      ...
      (let [result (#'cli/run-cli {:representation-dir (.getAbsolutePath tmp-dir) :level 2})]
        (testing ":library synonym-pairs reflects the normalized match"
          (is (=? {:value 1 :score 50}
                  (get-in result [:library :dimensions :semantic :variables :synonym-pairs]))))
...
(deftest ^:parallel embeddings-path-override-test
  (testing "explicit :embeddings-path resolves relative to the representation dir, not cwd,
            and a missing file fails fast instead of silently scoring with no embeddings"
    (let [tmp-dir (doto (java.io.File/createTempFile "emb-override-rep-" "")
      ...
      (let [dir-path (.getAbsolutePath tmp-dir)
            entities [(bare-entity 10 "Alpha" :table) (bare-entity 11 "Beta" :table)]]
        (testing "relative override picks the subdirectory file (1 synonym pair, not 0)"
          (let [{:keys [embedder]} (representation/load-dir dir-path :embeddings-path "embeddings/variant.json")
                result             (complexity/score-from-entities
                                    (catalog entities) (catalog []) embedder {:level 2})]
            (is (= 1 (get-in result [:library :dimensions :semantic :variables :synonym-pairs :value])))))
    ...
    (let [tmp-dir (doto (java.io.File/createTempFile "nested-rep-" "")
      ...
      (let [{:keys [library universe]} (representation/load-dir (.getAbsolutePath tmp-dir))]
        (testing ":library includes only the grandchild-collection content"
          (is (= #{"in_grandchild" "GrandchildMetric"} (set (map :name (:entities library))))))
        (testing ":universe includes everything (collection scope doesn't apply)"
          (is (= #{"in_grandchild" "in_outside" "GrandchildMetric" "OutsideMetric"}
                 (set (map :name (:entities universe))))))))))
...
(deftest ^:parallel audit-db-filter-test
  (testing "audit-db content is filtered from both :universe and :library — the live scorer derives
            :library from an already-audit-filtered superset, so audit content in a Library
            collection is dropped even though the collection scope alone would keep it."
    (let [tmp-dir  (doto (java.io.File/createTempFile "audit-rep-" "")
      ...
      (let [{:keys [library universe]} (representation/load-dir (.getAbsolutePath tmp-dir))]
        (testing ":universe excludes both audit table and audit card"
          (is (= #{"orders" "Revenue"} (set (map :name (:entities universe))))))
        (testing ":library excludes them too (library ⊆ universe, matching the live scorer)"
          (is (= #{"orders" "Revenue"} (set (map :name (:entities library))))))))))
...
(deftest ^:parallel nil-db-id-filtered-from-universe-test
  (testing "Rows with a nil :db_id / :database_id are dropped from :universe and :library — the
            live scorer uses Toucan `[:not= audit/audit-db-id]`, which compiles to SQL `<>` and
            excludes NULL-db rows via three-valued logic. `(not= nil audit-id)` would keep them,
            so the offline loader must use a nil-safe predicate to preserve the guarantee."
